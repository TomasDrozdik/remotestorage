package cz.cuni.mff.client;

import com.google.protobuf.InvalidProtocolBufferException;
import communication.ProtoCommunication;
import cz.cuni.mff.client.processors.UserRequestProcessor;
import remotestorage.proto.MessagesProtos;
import cz.cuni.mff.client.requests.AdminRequests;
import cz.cuni.mff.client.requests.HelpPrints;
import cz.cuni.mff.client.requests.UserRequests;
...
/**
 * Client represents one client connection to a server.
 * It is specified by input, output and error streams for UI and a Socket for communication with server.
 *
 * Clients implements Runnable to emphasize that it can run in separate thread only needing it's streams described
 * above. This is intended to be used in future GUI or for client side broker handling multiple clients at the same
 * time running in parallel.
 */
public class Client implements Runnable {
    ...
    private Request parseRequest(String line) {
        ...
        switch (tokens[0]) {
            ...
            case "?":
                ...
                break;
            case "exit":
                return new Request(new Request.EmptyRequestTag());
            default:
     ...
     */
    static class ParallelStreamRedirect extends Thread {
        InputStream in;
        PrintStream out;
